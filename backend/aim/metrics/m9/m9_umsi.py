...
class Metric(AIMMetricInterface):
    ...
    @classmethod
    def postprocess_predictions(cls, pred, shape_r, shape_c, blur=False, normalize=False):
        """
        Preprocess the image to the size required by the model.
    ...
    def execute_metric(
        ...
        # Create PIL image
        umsi_image: Image.Image = Image.open(BytesIO(base64.b64decode(gui_image)))
        umsi_image: Image.Image = umsi_image.convert("RGB")
        ...
        heat_map = cls.postprocess_predictions(preds[0][0,:,:,0], height, width)
