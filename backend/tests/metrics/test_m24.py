...
"""
Tests for the 'GUI Coverage' metric (m24).
"""
...
from aim.metrics.m24.m24_gui_coverage import Metric
...
@pytest.mark.parametrize(
    ...
    [
        ("interfacemetrics_aalto.png", [0.2086, 0.2086]),
        ("white.png", [0.0, 0.0]),
    ],
)
def test_gui_coverage_desktop(
    input_value: str, expected_results: List[Any]
) -> None:
    """
    Test GUI Coverage (desktop GUIs).
    ...
    # Test result
    if result is not None and all(isinstance(res, float) for res in result):
        assert [round(float(res), 4) for res in result] == expected_results
